...
export function NewsCard({ article }: { article: NewsArticle }) {
  return (
    <article className="flex h-full flex-col overflow-hidden rounded-card border border-border bg-white shadow-line">
      <PlaceholderImage alt={article.imageAlt} src={article.image} />
      <div className="flex flex-1 flex-col p-6">
        ...
        {article.isPlaceholder ? (
          <p className="mt-4 text-xs font-bold uppercase tracking-[0.14em] text-gold">
            Coming Soon
          </p>
        ) : null}
        <div className="mt-auto pt-6">
          <Link
            ...
            href={article.href}
          >
            {article.isPlaceholder ? "View Updates" : "Read Article"}
